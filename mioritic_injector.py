...
class MioriticInjector:
    ...
    def start_injection(self):
        """Start the injection process in a separate thread"""
    ...
    def analyze_and_inject(self, url, injection_word):
        ...
        try:
            ...
            text_elements = soup.find_all(['p', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'span', 'div', 'a'])
            ...
            for i, point in enumerate(injection_points[:5]):  # Limit to first 5
                self.log(f"Attempting injection {i+1}: {point['text'][:50]}...")
                
                # Simulate injection attempt
                modified_text = point['text'].replace('test', injection_word)
                if modified_text != point['text']:
                    self.log(f"✓ Successfully injected '{injection_word}' into text element")
